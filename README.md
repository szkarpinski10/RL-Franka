...
Task families live in `src/rl_franka/tasks`. Within a manager-based family, task-wide MDP terms live in `mdp`, while
robot-specific scenes, registrations, and agent configurations live in `config/franka`. Add another robot by
creating a sibling of `config/franka`; add another task by creating a sibling task-family directory.
